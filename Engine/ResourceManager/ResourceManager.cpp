...
void ResourceManager::load_model(string path,string name)
{
    ...
    MeshData<texture_vertex>*data=new MeshData<texture_vertex>();
    loader.load_model(*data);
    models_loaded[name]=data;
}
MeshData<texture_vertex>& ResourceManager::get_model(string name)
{
    MeshData<texture_vertex>&refx=*models_loaded[name];
    return refx;
}
void ResourceManager::delete_all_models()
{
    map<string,MeshData<texture_vertex>*>::iterator map_iterator;
