...
void ShaderProgram::attach_fragment_shader(FragmentShader v) const
{
    glAttachShader(shader_program_index, v.get_shader_index());
}
void ShaderProgram::attach_vertex_shader(VertexShader v) const
{
    glAttachShader(shader_program_index, v.get_shader_index());
}
void ShaderProgram::check_for_succes() const
{
    int success;
    char infoLog[512];
...
void ShaderProgram::set_mat4(glm::mat4 to_set,std::string& name) const
{
...
void ShaderProgram::set_float(float to_set,std::string& name) const
{
    int modelLoc = glGetUniformLocation(shader_program_index, name.c_str());
    glUniform1f(modelLoc, to_set);
}
